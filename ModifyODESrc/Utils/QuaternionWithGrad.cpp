...
void scale_vec3(double a, const double * b, double * c)
{
    c[0] = a * b[0];
    c[1] = a * b[1];
    c[2] = a * b[2];
...
void vec3_copy_vec3(const double * a, double * b)
{
    b[0] = a[0];
    b[1] = a[1];
    b[2] = a[2];
...
void vec3_add_vec3(const double * a, const double * b, double * c)
{
    c[0] = a[0] + b[0];
    c[1] = a[1] + b[1];
    c[2] = a[2] + b[2];
...
void vec3_minus_vec3(const double * a, const double * b, double * c)
{
    c[0] = a[0] - b[0];
    c[1] = a[1] - b[1];
    c[2] = a[2] - b[2];
...
void mat3_mat3_multiply_single(
    const double * a,
    const double * b,
    double * c
)
...
void mat3_mat3T_multiply_single(
    const double * a,
    const double * b,
    double * c
)
...
)
{
    for(size_t i=0; i<num_quat; i++)
    {
        quat_multiply_single_float32(q1 + 4 * i, q2 + 4 * i, q + 4 * i);
    }
}
...
)
{
    ...
    double dot_val = 0.0;
    double qd[4];
    for(int i = 0; i < 4; i++) dot_val += prev_q[i] * q[i];
    if (dot_val < 0)
    {
        for(int i = 0; i < 4; i++) qd[i] = inv_dt * (-q[i] - prev_q[i]);
    }
    else
    {
        for(int i = 0; i < 4; i++) qd[i] = inv_dt * (q[i] - prev_q[i]);
    }
    double q_conj[4] = {-prev_q[0], -prev_q[1], -prev_q[2], prev_q[3]};
    double res_q[4] = {0.0, 0.0, 0.0, 1.0};
    quat_multiply_single(qd, q_conj, res_q);
    for(int i = 0; i < 3; i++) result[i] = 2 * res_q[i];
}

void delta_quat2_single(
    const double* prev_q,
    const double* q,
    double inv_dt,
    double* result
...
void delta_quat2_single_float32(
    const float* prev_q,
    const float* q,
    float inv_dt,
    float* result
)
{
    ...
    float q_conj[4] = {-prev_q[0], -prev_q[1], -prev_q[2], prev_q[3]};
    float res_q[4] = {0, 0, 0, 1};
    quat_multiply_single_float32(qd, q_conj, res_q);
    for(int i = 0; i < 3; i++) result[i] = 2 * res_q[i];
}

void delta_quat_impl(
    const double* prev_q,
    const double* q,
    double dt,
    double* result,
...
)
{
    for(size_t i = 0; i < num_quat; i++)
    {
        delta_quat_single(prev_q + 4 * i, q + 4 * i, dt, result + 4 * i);
    }
}
...
)
{
    for(size_t i = 0; i < num_quat; i++)
    {
        delta_quat2_single_float32(prev_q + 4 * i, q + 4 * i, inv_dt, result + 3 * i);
    }
}
...
void quat_between_single(const double* a, const double* b, double* result) // TODO: check result..
{
    double cross_res[3];
    cross_product_single(a, b, cross_res);
    double alen2 = a[0] * a[0] + a[1] * a[1] + a[2] * a[2];
    double blen2 = b[0] * b[0] + b[1] * b[1] + b[2] * b[2];
    double ab    = a[0] * b[0] + a[1] * b[1] + a[2] * b[2];
    double w_ = std::sqrt(alen2 * blen2) + ab;
    double res[4] = { cross_res[0], cross_res[1], cross_res[2], w_ };
    double ratio = 1 / std::sqrt(res[0] * res[0] + res[1] * res[1] + res[2] * res[2] + res[3] * res[3]);
...
double my_clamp(double x, double l, double r)
{
    if (x > r) x = r;
    else if (x < l) x = l;
    return x;
...
// decompose quaternion q along axis vb
// lenght of q is 4
// length of vb, result are 3
