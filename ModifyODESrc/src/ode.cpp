...
void dBodySetQuaternionWrapper2(dBodyID b, double x, double y, double z, double w)
{
  dAASSERT (b);
  b->q[0] = w;
  b->q[1] = x;
...
Quat4Wrapper dBodyGetQuaternionWrapper (dBodyID b)
{
  dAASSERT (b);
  return { b->q[1], b->q[2], b->q[3], b->q[0]};
}
...
Mat33Wrapper dBodyGetInertiaWrapper(dBodyID b)
{
    dAASSERT(b);
    dMatrix3 tmp, odeMat3;
    // compute inverse inertia tensor in global frame
    
    dMultiply2_333(tmp, b->mass.I, b->posr.R);
    dMultiply0_333(odeMat3, b->posr.R, tmp); // R * I * R^T
    return {odeMat3[0], odeMat3[1], odeMat3[2],
            odeMat3[4], odeMat3[5], odeMat3[6],
            odeMat3[8], odeMat3[9], odeMat3[10]};
}
...
Mat33Wrapper dBodyGetInertiaInvWrapper(dBodyID b)
{
  dAASSERT(b);
    dMatrix3 tmp, odeMat3;
    // compute inverse inertia tensor in global frame
    ...
    dMultiply2_333(tmp, b->invI, b->posr.R);
    dMultiply0_333(odeMat3, b->posr.R, tmp); // R * invI * R^T
    return {odeMat3[0], odeMat3[1], odeMat3[2],
            odeMat3[4], odeMat3[5], odeMat3[6],
            odeMat3[8], odeMat3[9], odeMat3[10]};
}
