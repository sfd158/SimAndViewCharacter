...
static dWorldID dsWorld = NULL;
static dSpaceID dsDebugSpace = NULL;
...
static std::vector<dGeomID> vis_geoms_list;

static const int ds_window_width = 1280;
static const int ds_window_height = 720;
...
struct dsVideoRecorder{
	unsigned char buffer[max_buffer_size][ds_window_width * ds_window_height * 3];
	int frame_count = 0;
	...
	static void recordFrame(unsigned char * buf)
	{
	...
	void appendFrame()
	{
		if (frame_count >= max_buffer_size)
		{
			return;
		}
		unsigned char * mem = buffer[frame_count++];
		recordFrame(mem);
	}

	inline int is_enable()
	{
		return enable_flag;
	}
...
void dsWorldSetter(dWorldID value, dSpaceID space, std::vector<dGeomID> vis_list_)
{
	dsWorld = value;
	dsDebugSpace = space;
	vis_geoms_list = vis_list_;
	std::cout << dsWorld << " " << dsDebugSpace << std::endl;
}
...
static void render_single_geom(dGeomID g, int & meshCnt)
{
	static float pos[3];
	static float rot[12];
...
void dsDrawStep(int pause)
{
	...
	for(dBodyID b = dWorldGetFirstBody(dsWorld); b != NULL; b = dWorldGetNextBody(b))
	{
		for(dGeomID g = dBodyGetFirstGeom(b); g != NULL; g = dBodyGetNextGeom(g))
		{
			render_single_geom(g, meshCnt);
		}
	}
	// here we can render center of mass..

	if (dsDebugSpace != NULL)
	{
		for(dGeomID g = dSpaceGetFirstGeom(dsDebugSpace); g != NULL; g = dSpaceGetNextGeom(g))
			render_single_geom(g, meshCnt);
	}
	for(int i = 0; i < vis_geoms_list.size(); i++)
	{
		render_single_geom(vis_geoms_list[i], meshCnt);
	}
	if (HINGEAXIS)
	{
		for (int k = 0; k < 12; k++)
...
void dsDrawWorld()
{
	...
	// run simulation
	dsSimulationLoop(0, NULL, ds_window_width, ds_window_height, &fn); // we need also add sync mode..
}
...
void dsEndRecordVideo(unsigned char * data, size_t num_frame)
{
	...
	if (data != nullptr)
	{
		memcpy(data, video_recorder.buffer, (sizeof(unsigned char) * 3 * ds_window_width * ds_window_height) * video_recorder.frame_count);
	}
